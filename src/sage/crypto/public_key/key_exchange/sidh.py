...
class SIDH(KeyExchangeBase):
    ...
    def alice_compute_shared_secret(self, alice_secret_key, bob_public_key):
        phi_A1 = self.alice_second_secret_isogeny(
            self, alice_secret_key, bob_public_key
        )
    ...
    def bob_first_secret_isogeny(self, bob_secret_key):
        isogenyMap = self.buildIsogenyByBreakingDown(
            bob_secret_key, 3, self._E, self._P_B, self.Q_B
        )
        return isogenyMap[0]
    ...
    def alice_second_secret_isogeny(self, alice_secret_key, bob_public_key):
        ...
        isogenyMap = self.buildIsogenyByBreakingDown(
            alice_secret_key, 2, E_B, P_A1, Q_A1
        )
        return isogenyMap[0]
    ...
    def buildIsogenyByBreakingDown(
        self, person_secret_key, ell, domain_EC, P_generate, Q_generate
    ):
        sequenceOfIsogenies = []
        if ell == 2:
            e_power = self._e_A
